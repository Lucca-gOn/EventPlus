...
.AddJwtBearer("JwtBearer", options =>
{
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        IssuerSigningKey = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes("chave-autenticacao-code-first-webapi-projeto-event+")),

        //Valida o tempo de expiração do token
        ClockSkew = TimeSpan.FromMinutes(10),

        //Nome do issuer (de onde esta vindo)
        ValidIssuer = "webapi.event+.codeFirst",

        //Nome do issuer (para onde esta indo)
        ValidAudience = "webapi.event+.codeFirst"
    };
});
...
builder.Services.AddSwaggerGen(options =>
{
    ...
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        Version = "v1",
        Title = "API Event+",
        Description = "API para o gerenciamentos de eventos - Event+ WEB.API",
